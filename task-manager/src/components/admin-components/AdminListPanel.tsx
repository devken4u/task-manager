...
import { getAdminListQuery } from "../../api/queries/UserQuery";
import {
...
export default function AdminListPanel() {
  ...
  const getAdminList = getAdminListQuery(
    itemsPerPage,
    currentPage,
    sortBy,
    similar
  ...
  return (
    <div className="p-4 pb-2 ">
      <div className="pb-4 overflow-x-auto">
        <div className="flex justify-between min-w-[500px] mb-4">
          <SearchSimilar
          ...
          />
          <Button
            className="px-4 font-semibold bg-blue-500 rounded-md text-zinc-50"
            onClick={() => {
              setIsNewAdminTabOpen(true);
            }}
          >
            Add Admin
          </Button>
        ...
        <table className="min-w-full mb-2 text-sm text-left text-gray-500 border border-collapse border-gray-300">
          ...
          <tbody className="divide-y divide-gray-300">
            {getAdminList.data?.adminInformationList.map((user, index) => (
        ...
        {getAdminList.data?.adminInformationList.length === 0 && (
          <p className="my-4 text-center">No data found.</p>
        )}
        {getAdminList.isLoading && (
        ...
        <PageSelection
          itemsPerPage={itemsPerPage}
          totalItems={getAdminList.data?.totalAdmin || 0}
