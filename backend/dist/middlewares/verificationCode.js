...
const checkVerificationCode = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    // get the code from the body
    const { code = "" } = req.body;
    // get the account from the body (the source of this is the authenticate)
    ...
    if (!verificationInformation) {
        res.status(400).json({
            message: "It's either there is no verification code requested or it is already expired.",
